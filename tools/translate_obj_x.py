...
def transform_v_line(line_bytes, offset):
    """Translate the X coordinate of a v line, preserving the rest.

    Adds the offset to the X (first numeric token) only.
    Preserves the leading keyword, Y and Z coordinates, any extra trailing
...
def main(argv):
    if len(argv) != 4:
        sys.stderr.write(
            "Usage: python translate_obj_x.py <input.obj> <offset> <output.obj>\n"
        )
        return 2
    input_path = argv[1]
    try:
