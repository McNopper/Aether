...
def transform_v_line(line_bytes, offset):
    """Translate the Y coordinate of a v line, preserving the rest.

    Adds the offset to the Y (second numeric token) only.
    Preserves the leading keyword, X and Z coordinates, any extra trailing
    ...
    y = float(coords[1]) + offset  # translate Y only
    z = coords[2]  # keep Z unchanged (as string token)

    new_parts = [keyword, x, fmt(y), z]
...
def main(argv):
    if len(argv) != 4:
        sys.stderr.write(
            "Usage: python translate_obj_y.py <input.obj> <offset> <output.obj>\n"
        )
        return 2
    input_path = argv[1]
    try:
